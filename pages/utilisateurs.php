...
$error = '';
try {
    $stmt = $pdo->query("SELECT id_utilisateur, nom_utilisateur, role, date_creation FROM utilisateur ORDER BY id_utilisateur DESC");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (isset($_POST['add'])) {
        ...
        if ($nom_utilisateur && $mot_de_passe && $role) {
            ...
            try {
                $stmt = $pdo->prepare("INSERT INTO utilisateur (nom_utilisateur, mot_de_passe, role) VALUES (?, ?, ?)");
                $stmt->execute([$nom_utilisateur, $hash, $role]);
    ...
    } elseif (isset($_POST['delete'], $_POST['id_utilisateur'])) {
...
<title>Utilisateurs - Gestion Flottes</title>
...
<div class="main-wrapper d-flex">
    <?php renderSidebar('utilisateurs'); ?>
        ...
        <div class="modal-content">
            <form method="POST">
                ...
                <div class="modal-body">
                    <input type="text" name="nom_utilisateur" placeholder="Nom utilisateur" class="form-control mb-2" required>
                    <input type="password" name="mot_de_passe" placeholder="Mot de passe" class="form-control mb-2" required>
